...
def convert_label_to_similarity(normed_feature: Tensor, label: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    similarity_matrix = 1 - (1 - similarity_matrix.pow(2)).clamp(1e-6, 1).sqrt()        # 使用1-sin替代余弦距离
...
class CircleLoss(nn.Module):
    def __init__(self, m=0.25, gamma=256.0) -> None:
        super(CircleLoss, self).__init__()
        self.m = m
        self.gamma = gamma
        self.soft_plus = nn.Softplus()

    def forward(self, feature, labels) -> Tensor:
        ...
        loss = self.soft_plus(torch.logsumexp(logit_n, dim=0) + torch.logsumexp(logit_p, dim=0)) / self.gamma
