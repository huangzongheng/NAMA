...
class MSMT17(BaseImageDataset):
    ...
    def __init__(self, root, verbose=True, combine_all=False, **kwargs):
        ...
        if combine_all:
            train = sum([self._process_dir(x, relabel=True) for x in [self.train_dir, self.query_dir, self.gallery_dir]], [])
        else:
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        pattern = re.compile(r'([-\d]+)_c(\d{1,2})')
